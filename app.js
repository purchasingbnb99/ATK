...
(function () {
  ...
  function addFinalReqRow(){var c=byIdFinal('finalReqItems'),r=document.createElement('div');r.className='request-item-grid fcr-row';r.innerHTML='<div class="form-group"><label>Barang</label><select class="field fcr-product" required>'+productOptions()+'</select></div>'+numberClassFinal('fcr-qty','Qty',1)+'<div class="form-group"><label>Catatan Item</label><input class="field fcr-note" maxlength="300" placeholder="Catatan untuk barang ini"></div><div class="form-group request-remove-field"><label>&nbsp;</label><button type="button" class="btn btn-danger fcr-remove">Hapus</button></div>';c.appendChild(r);r.querySelector('.fcr-remove').onclick=function(){r.remove();};}
